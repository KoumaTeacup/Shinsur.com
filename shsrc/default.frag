...
//	vec4 diffuse = UseRawColor ? RawColor : texture(DiffuseSampler, UV.xy);
	vec4 diffuse = texture(DiffuseSampler, UV.xy);
	vec4 light = vec4((vec3(max(dot(Normal, L),0.0f)) + vec3(0.2,0.2,0.2)), 1.0f);
	outColor  = light * diffuse; //(dis/10000000.0);
